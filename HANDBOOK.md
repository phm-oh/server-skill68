...
1. **สร้าง/แก้ไข `/etc/docker/daemon.json`:**
   ...
   **⚠️ สำคัญ:** ต้องระบุ IP ของ GitLab server (192.168.100.130:5050)
   
   **แบบ Minimal (แนะนำ - ง่ายที่สุด):**
   ```bash
   ...
   **แบบ Full (ถ้าต้องการ log options และ domain name):**
   ```bash
   sudo mkdir -p /etc/docker
   sudo tee /etc/docker/daemon.json > /dev/null <<EOF
   {
   ...
   **คำอธิบาย:**
   - `"192.168.100.130:5050"` - **จำเป็น** IP ของ GitLab Registry (ต้องมี)
   - `"gitlab.local:5050"` - **Optional** ใช้ได้ถ้ามี hosts entry (ถ้าไม่มีก็ไม่ต้องใส่)
   - `"localhost:5050"` - **Optional** ใช้ได้ถ้า Registry อยู่บนเครื่องเดียวกัน (ส่วนใหญ่ไม่ต้องใส่)
   - `log-driver` และ `log-opts` - **Optional** สำหรับจัดการ Docker logs
   
   **💡 แนะนำ:** ใช้แบบ Minimal (ใส่แค่ IP หลัก) ก็เพียงพอแล้ว
...
4. **ทดสอบ login ไป Registry:**
   ...
   **⚠️ สำคัญ:** ต้อง login เพื่อให้สามารถ pull images จาก GitLab Registry ได้
   
   **ขั้นตอน:**
   
   a. **วิธีที่ 1: ใช้ Personal Access Token (แนะนำ - ปลอดภัยกว่า)**
      
      **สร้าง Personal Access Token:**
      1. เปิด browser: `http://192.168.100.130`
      2. Login เข้า GitLab
      3. ไปที่ **User Settings** (คลิกที่ avatar มุมบนขวา → **Preferences**)
      4. เลือก **Access Tokens** (เมนูซ้าย)
      5. ตั้งชื่อ Token: `docker-registry` (หรือชื่ออื่น)
      6. เลือก **Scopes:**
         - ✅ `read_registry` (สำหรับ pull images)
         - ✅ `write_registry` (สำหรับ push images)
      7. เลือก **Expiration date** (หรือไม่กำหนด)
      8. คลิก **Create personal access token**
      9. **⚠️ สำคัญ:** Copy token ที่ได้มาเก็บไว้ (จะแสดงแค่ครั้งเดียว!)
         - Token จะมีรูปแบบ: `glpat-xxxxxxxxxxxxxxxxxxxx`
      ...
      **กรอกข้อมูล:**
      ```
      Username: root
      Password: glpat-xxxxxxxxxxxxxxxxxxxx  (วาง token ที่ copy มา)
      ```
   
   b. **วิธีที่ 2: ใช้ Username + Password (บางครั้งใช้ไม่ได้)**
      
      **⚠️ หมายเหตุ:** GitLab บางเวอร์ชันอาจไม่อนุญาตให้ใช้ password โดยตรง
      ...
      **⚠️ ถ้าเจอ error "unauthorized" หรือ "invalid credentials":**
      - ใช้วิธีที่ 1 (Personal Access Token) แทน
   
   c. **วิธีที่ 3: ใช้ Deploy Token (สำหรับ Project-specific)**
      
      **สร้าง Deploy Token:**
      1. ไปที่ Project → **Settings** → **Repository** → **Deploy tokens**
      2. ตั้งชื่อ: `docker-registry`
      3. เลือก **Scopes:**
         - ✅ `read_registry`
         - ✅ `write_registry`
      4. เลือก **Expiration date**
      5. คลิก **Create deploy token**
      6. **⚠️ สำคัญ:** Copy Username และ Token ที่ได้มา
         - Username: `gitlab+deploy-token-XX` (หรือตามที่แสดง)
         - Token: `xxxxxxxxxxxxxxxxxxxx`
      
      **Login ด้วย Deploy Token:**
      ```bash
      docker login 192.168.100.130:5050
      ```
      ...
      **กรอกข้อมูล:**
      ```
      Username: gitlab+deploy-token-XX  (username ที่ได้จาก Deploy Token)
      Password: xxxxxxxxxxxxxxxxxxxx  (token ที่ได้จาก Deploy Token)
      ```
   
   d. **ตรวจสอบว่า login สำเร็จ:**
      ```
      Login Succeeded
      ```
      
      **✅ ถ้าเห็น "Login Succeeded" แสดงว่าสำเร็จ!**
   
   e. **ตรวจสอบ credentials ที่เก็บไว้:**
      ```bash
      cat ~/.docker/config.json
      # ควรเห็น auth สำหรับ 192.168.100.130:5050
      ```
   
   **💡 Tip:** Credentials จะถูกเก็บไว้ใน `~/.docker/config.json` และจะใช้ได้จนกว่าจะ logout
   
   **❓ คำถาม: หลัง reboot server ต้อง login ใหม่ไหม?**
   
   **คำตอบ:** **ไม่ต้อง!** Credentials จะถูกเก็บไว้ใน `~/.docker/config.json` และจะใช้ได้ต่อไปแม้หลัง reboot server หรือ restart Docker service
   
   **⚠️ ต้อง login ใหม่เมื่อ:**
   - Logout ด้วยคำสั่ง `docker logout`
   - ลบไฟล์ `~/.docker/config.json`
   - Token หมดอายุ (ถ้าใช้ Personal Access Token)
   - เปลี่ยน password หรือ token
   
   **ถ้าต้องการ logout:**
      ```bash
      docker logout 192.168.100.130:5050
      ```
...
2. **Login Registry (สำคัญมาก!):**

   **⚠️ ต้อง login เพื่อให้สามารถ pull images จาก GitLab Registry ได้**
   ...
   **ขั้นตอน:**
   ...
   e. **ทดสอบ pull image (ถ้ามี):**
      ```bash
      docker pull 192.168.100.130:5050/root/devops-project/backend:latest
      # หรือ
      docker pull 192.168.100.130:5050/root/devops-project/frontend:latest
      ```
      
      **⚠️ ถ้าเจอ error "unauthorized" แสดงว่า login ไม่สำเร็จ ให้ login ใหม่**
...
**⚠️ สำคัญ:** Backend ไม่ใช้ MQTT client โดยตรง ใช้ Node-RED เป็นตัวกลาง

**Flow:** ESP32 → MQTT Broker → Node-RED → HTTP POST → Backend

1. **เข้า Node-RED:**
   - เปิด browser: `http://192.168.100.131:1880`
   - Login: `admin` / `admin123`

2. **ตั้งค่า Node-RED Flow:**
   
   **สร้าง Flow สำหรับแต่ละห้อง (room1, room2, room3):**
   
   a. **เพิ่ม MQTT In Node:**
      - ลาก **mqtt in** จาก palette มาวาง
      - Double-click เพื่อแก้ไข
      - **Topic:** `room1/temperature` (สำหรับ room1)
      - **Broker:** คลิกปุ่ม edit ด้านขวา → **Add new mqtt-broker**
        - **Server:** `mqtt` (หรือ `192.168.100.131`)
        - **Port:** `1883`
        - คลิก **Update** แล้ว **Add**
      - ตรวจสอบว่าเชื่อมต่อได้ (สีเขียว "Connected")
   
   b. **เพิ่ม HTTP Request Node:**
      - ลาก **http request** จาก palette มาวาง
      - Double-click เพื่อแก้ไข
      - **Method:** `POST`
   ...
   c. **เชื่อมต่อ:** MQTT In → HTTP Request
      - ลากเส้นจาก MQTT In ไป HTTP Request
   
   d. **ทำซ้ำสำหรับ room2 และ room3:**
      - **room2:** Topic `room2/temperature` → URL `http://backend:3000/api/mqtt/room2`
      - **room3:** Topic `room3/temperature` → URL `http://backend:3000/api/mqtt/room3`
   
   e. **Deploy Flow:** คลิก **Deploy** (ปุ่มมุมบนขวา)
      - ควรเห็นข้อความ "Successfully deployed"
...
4. **ตรวจสอบผลลัพธ์:**
   - **ดูใน Node-RED:** ควรเห็น message ผ่าน flow (debug node)
   - **ตรวจสอบ Backend logs:**
     ```bash
     docker logs backend
     # ควรเห็น "Saved temp from Node-RED: Room 1 = 25.5°C"
     ```
   - **ตรวจสอบ Database:**
     - ผ่าน phpMyAdmin: `http://192.168.100.131:8081`
     - หรือ SQL: `SELECT * FROM temperatures ORDER BY id DESC LIMIT 5;`
