...
app.get('/users', auth(['admin']), async (req, res) => {
    ...
    // Attach room access
    for (let u of users) {
        const [access] = await db.query('SELECT room_id FROM room_access WHERE user_id = ?', [u.id]);
        u.roomIds = access.map(a => a.room_id);
    }
    res.json(users);
});
...
app.put('/users/:id', auth(['admin']), async (req, res) => {
    try {
        ...
        let params = [req.body.role, req.params.id];

        // Update Password if provided
        if (req.body.password) {
            const hash = bcrypt.hashSync(req.body.password, 10);
            sql = 'UPDATE users SET role = ?, password = ? WHERE id = ?';
            params = [req.body.role, hash, req.params.id];
        }
        
        await db.query(sql, params);

        // Update Room Access (Clear old, Add new)
        if (req.body.roomIds && Array.isArray(req.body.roomIds)) {
            await db.query('DELETE FROM room_access WHERE user_id = ?', [req.params.id]);
            for (const roomId of req.body.roomIds) {
                await db.query('INSERT INTO room_access (user_id, room_id) VALUES (?, ?)', [req.params.id, roomId]);
            }
        }
        res.sendStatus(200);
    } catch (e) { res.status(500).send(e.message); }
